...
class Reports_model extends CI_Model {
	...
	public function show_stock_report(){
		...
		$this->db->select("b.tax_name");
			//If Brnach Login
				if($role_id != 1 || $role_id != 2 || $role_id != 36 || $role_id != 37 || $role_id != 42 || $role_id != 46 || $role_id != 47 || $role_id != 49){
				    $this->db->from("db_warehouseitems e");
        		    $this->db->join("db_items a", "e.item_id = a.id", "left");
		...
		$this->db->join("db_tax as b","a.tax_id=b.id","left");
...
//		$this->db->group_by("c.`item_id`");
	...
	public function show_stock_transfer_report(){
		...
		$this->db->select("c.transfer_qty,d.item_name,d.mrp");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$total_amount = $res1->transfer_qty*$res1->mrp;
				...
				echo "<td>".round($res1->mrp,2)."</td>";
				echo "<td>".$total_amount."</td>";
				echo "</tr>";
				$tot_transfer_qty+=$res1->transfer_qty;
				$totl_amount+=$total_amount;
			...
			echo "<tr>
					  ...
					  <td></td>
					  <td class='text-left text-bold'>".number_format($totl_amount,2)."</td>
				  </tr>";
		}
		else{
			$total_columns_count=9;

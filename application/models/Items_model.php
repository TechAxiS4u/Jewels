...
class Items_model extends CI_Model {
	...
	{	
		...
		if(!empty($warehouse_id)){
			//This is newly added Code for Customise
			$this->db->from('db_warehouseitems as w');
			$this->db->where('a.id=w.item_id');
			$this->db->where('w.warehouse_id',$warehouse_id);
		}
	...
	public function save_record($modal_post=array()){
		...
		if($item_group=='Single'){
			...
			//Opening Stock Exist
			if($adjustment_qty>0 || $adjustment_qty<0){
				...
				for($i=1;$i<=$existing_row_count;$i++){
					if(isset($_REQUEST['tr_variant_id_'.$i]) && !empty($_REQUEST['tr_variant_id_'.$i])){
						...
						$info = array(
							...
							//Opening Stock Exist
							if($opening_stock>0 || $opening_stock<0){
								$array_params = array(  'store_id'			=> 	$store_id,
	...
	public function preview_labels(){
		...
		<div style=" height:5in !important;  width:8.5in !important; line-height: 16px !important;margin-top: 15px;">
			<div class="inner-div-2" style=" height:11in !important;  width:6.5in !important; line-height: 16px !important;margin-top: 0.2in !important;">
				<div style="">
					...
					for($i=1;$i<=$rowcount;$i++){
						...
						if(isset($_POST['tr_item_id_'.$i]) && !empty($_POST['tr_item_id_'.$i])){
							...
							?>
							<style type="text/css">
								.label_border:nth-child(even) {
									margin-left:30px;
								}

								.label_border::nth-child(n+3) {
    					margin-top:25px; /* Adjust the value as needed */
  								}

  								/*.label_border:nth-child(-n+2) {
    					margin-top: -10px; 
								}*/
							</style>
							
							<div style="height:1.5in !important; line-height: 1in; width:2in !important; display: inline-block;margin-right: 15px; <?=$page_break;?>  " class="label_border text-center">
							<div style="display:inline-block;vertical-align:middle;line-height:16px !important;text-align:center;">
							<img src="<?php echo base_url(); ?>uploads/company/LeedsLogon.png" style="width: 120px;">
								<!-- <b style="display: block !important" class="text-uppercase"><?=$store_name;?></b> -->
									<p style="padding-top: 5px;">
										<b><span style="display: block !important;font-size: 17px;">
									CODE: <?= $item_codes;?>
									</span>
								</b>
							</p>
								<b>
								<span style="font-size: 17px;">MRP: <?= $CI->currency($item_price);?></span>
								<b style="display: block !important" class="text-uppercase"><?=$store_name;?>
								</b>

		...
		$role_id = $this->session->userdata('role_id');
		  ...
		  $this->db->select("a.*,b.tax,b.tax_name");
				...
				if($role_id != 1 || $role_id != 2 || $role_id != 36 || $role_id != 37 || $role_id != 42 || $role_id != 46 || $role_id != 47 || $role_id != 49){
				    $this->db->from("db_warehouseitems c");
        		    $this->db->join("db_items a", "c.item_id = a.id", "left");
				...
				$this->db->join(" db_tax b ","a.tax_id=b.id",'left');
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	        	...
	        	if($w_stock <1 && !$service_bit){
	        		$str="zero_stock()";
	        		$disabled='';
	        		$bg_color="background-color:#9d9999;display:none;";
	        	}
	        	else{
	        		$str="addrow($res2->id)";
	...
	public function pos_save_update(){//Save or update sales
		...
		if($command=='update'){
			...
			$sales_entry = array(
		    				...
		    				'status' 					=> 1,
		    				'sales_note' 				=> $sales_note,
		    				'created_by_address' 		=> $CUR_USER_ADDRESS,
		    			);
			$sales_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();

<?php
  ...
  function total_worth_of_stk_items($stocktransfer_id){
    $CI =& get_instance();
    // if(empty($store_id)){
    //   $store_id= get_current_store_id();
    // }
    $CI->db->select("COALESCE(sum(transfer_qty),0) as transfer_qty,item_id")->where("store_id",get_current_store_id())->from("db_stocktransferitems")->where("stocktransfer_id",$stocktransfer_id)->group_by("item_id");
    $q1 = $CI->db->get();
    $tot_sales_price=0;
      foreach ($q1->result() as $res1) {
        $item_price = $CI->db->select("coalesce((sales_price),0) as sales_price")->where("id",$res1->item_id)->get("db_items")->row()->sales_price;
        $tot_sales_price+=$item_price*$res1->transfer_qty;
      }
    return $tot_sales_price;
  }

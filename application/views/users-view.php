...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    $userwarehouseid = $this->db->query("SELECT warehouse_id from  db_userswarehouses where user_id='$userid'");
    foreach ($userwarehouseid->result() as $res2)
    {
    $comma_separated_data .= $res2->warehouse_id . ',';
    }
    $comma_separated_data = rtrim($comma_separated_data, ',');
  
        //if($usernm=='LEEDS')
        if($userRoleID=='1' || $userRoleID=='2' || $userRoleID=='36' ||$userRoleID=='47' )
        {
          ...
          //$this->db->where(" role_id not in (2)");
          //$this->db->where("a.store_id",get_current_store_id());
          $q1=$this->db->query("SELECT u.*,ur.role_name FROM db_users u inner join db_roles ur on ur.id=u.role_id inner join db_userswarehouses uw on u.id=uw.user_id where uw.warehouse_id in ($comma_separated_data) and u.id <> $userid and u.username <> 'LEEDS' group by uw.user_id");
}

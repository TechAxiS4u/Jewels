...
$usernm = $this->session->userdata('inv_username');
         
if($usernm=='LEEDS')
{
          $q1= $this->db->select("*")->get("db_warehouse");
}else {
         $q1= $this->db->query("select w.* from db_warehouse w inner join db_userswarehouses uw on uw.warehouse_id=w.id and uw.user_id='$userid'");
}
...
if($q1->num_rows() > 0) {
    ...
    foreach ($q1->result() as $res1) {
        $v = total_worth_of_warehouse_items($res1->id);
        $sum += $v;
        $vq= total_available_qty_items_of_warehouse($res1->id);
        $sumqty += $vq;
    }
}
?>
...
<p></p>
              ...
              <div class="small-box bg-3">
                <div class="inner">
                  <h3 class=""><?= $CI->currency(); ?> <?php echo $sum; ?></h3>
                  <p>Total Warehouse Worth</p>
                </div>
      ...
      <?php if(!is_user() && $CI->permissions('dashboard_info_box_2')){ ?> 
         <div class="row">
           ...
           <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box " >
            <span class="info-box-icon bg-5"><i class="fa fa-rupee"></i></span>
            <div class="info-box-content">
              <span class="info-box-text" style="font-size: 25px;"><?php echo $sum; ?></span>
              <span class="info-box-number">Total Worth</span>
            </div>
       ...
       <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box " >
            ...
            <div class="info-box-content">
              <span class="info-box-text" style="font-size: 25px;"><?php echo $sumqty; ?></span>
              <span class="info-box-number"><?= $this->lang->line('items'); ?></span>
            </div>
